...
function walk(dir, out = []) {
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (
      e.isFile() &&
      e.name === "page.tsx" &&
      full.toLowerCase().includes(`${path.sep}timeline${path.sep}`) &&
      !full.toLowerCase().includes(".bak")
    ) {
      out.push(full);
    }
  }
...
if (!files.length) {
  console.error("No real timeline/page.tsx files found.");
  process.exit(1);
}
...
console.log("DONE. Timeline should now start with Gold Nexus Alpha / Deep ML Academic Fusion.");
